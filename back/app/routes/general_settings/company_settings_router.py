...
from app.models.company_settings_model import Company_settingsCreateSchema, Company_settingsSchema, Company_settings
...
async def get_itm(company_settings_id:int,db:Session=Depends(get_db)):
    try:
        u=db.query(company_settings_id).filter(Company_settings.id == company_settings_id).first()
        return (u)
    except: 
        return HTTPException(status_code=422, details="Company Settings data not found")
