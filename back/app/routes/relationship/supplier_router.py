...
async def create(supplier: SupplierCreateSchema, db: Session = Depends(get_db)):
    srv = Supplier(supplier_name=supplier.supplier_name, supplier_email=supplier.supplier_email,
...
@supplier_router.get("/bmitvat/api/supplier/all_supplier", response_model=List[SupplierSchema],
...
@supplier_router.delete("/bmitvat/api/supplier/delete_supplier/{supplier_id}", response_class=JSONResponse,
                        dependencies=[Depends(get_current_active_user)])
async def get_itm(supplier_id: int, db: Session = Depends(get_db)):
    try:
        u = db.query(Supplier).filter(Supplier.id == supplier_id).first()
...
@supplier_router.put("/bmitvat/api/update_supplier_array/{supplier_id}",
                     dependencies=[Depends(get_current_active_user)])
