...
//                                             <input id="browserLname" type="text" value={s_address} className="form-input" required />
//                                         </div>
//                                         <div>
...
//                                                <td align="left"><strong><input type="text" id="grandTotal" disabled/></strong>
//                                                   <p></p>
//                                                </td>
//                                             </tr>
//                                          </table>
...
const addLocalPurchase = () => {
    ...
    const getItemByKeyUp = async (event: React.FormEvent<HTMLInputElement>) => {
        const searchInput = event.currentTarget as HTMLInputElement;
        const suggestionsList = document.getElementById('suggestionsList');
        ...
        if (token) {
            ...
            try {
                ...
                suggestions.forEach(suggestion => {
                    ...
                    listItem.style.padding = '10px';
                    listItem.className = 'suggestion-item';
                    listItem.value = suggestion.id.toString();
                    listItem.textContent = suggestion.item_name;
                    suggestionsList.appendChild(listItem);
                });
                ...
                selectedLiElements.forEach(async (liElement) => {
                    liElement.addEventListener('click', () => {
                        const clickedValue = parseInt((liElement as HTMLLIElement).value);
                        if (suggestionsList) {
                            suggestionsList.style.display = 'none';
                        }
    ...
    function addRow(data: any) {
        ...
        function calculateValue() {
            ...
            const totalAmount = vatAblValue + vatAmout;
            input10.value = totalAmount.toString();
        }
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const Vat = TotalVat ? TotalVat.value : '';
        const SD = TotalSD ? TotalSD.value : '';
        const ALL = AllTotal ? AllTotal.value : '';
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-6 gap-5">
                                    <div>
                                        ...
                                        <select id="getSupplier" onChange={getSupplierId} className="form-select text-dark col-span-4 text-sm" required >
                                            <option>Select Supplier</option>
                                            {all_suppliers.map((option, index) => (
                                                <option key={index} value={option.id}>{option.supplier_name}</option>
                                            ))}
                                        </select>
                                    </div>
                                    <div>
